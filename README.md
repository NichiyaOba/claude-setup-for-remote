...
```bash
# <owner> と <repo> はご自身の GitHub ユーザー名とリポジトリ名に置き換えてください
WORKDIR="$(mktemp -d /tmp/setup.XXXXXX)" && git clone https://github.com/<owner>/<repo>.git "${WORKDIR}" && bash "${WORKDIR}/main.sh"
```
