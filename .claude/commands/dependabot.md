...
**すべての GitHub API 操作は `gh` CLI を使用する。** MCP ツールは使用しない。

`gh` CLI が認証済みであることを前提とする。認証エラーが発生した場合のみ `AskUserQuestion` でユーザーに報告する。
...
- `gh pr view <PR番号> --repo <owner/repo> --json number,title,url,headRefName,state,author`
- そのPRのみを対象とする

**ブランチ名が指定された場合** (例: `dependabot/npm_and_yarn/lodash-4.17.21`):
- `gh pr list --repo <owner/repo> --head <ブランチ名> --json number,title,url,state,author,headRefName`

**リポジトリURLが指定された場合** (例: `https://github.com/owner/repo`):
- `gh pr list --repo <owner/repo> --author app/dependabot --state open --json number,title,url,headRefName`
...
```
Agent(model="claude-sonnet-4-5", tools=["Bash", "WebSearch", "WebFetch", "Read", "Glob", "Grep"], prompt="
...
**すべての GitHub API 操作は `gh` CLI を使用する。MCP ツールは一切使用しない。**
...
`gh pr view {PR_NUMBER} --repo {OWNER}/{REPO} --json state,mergeable` でPRの状態を確認する。
...
3. `git merge origin/{DEFAULT_BRANCH}` でデフォルトブランチをマージする
   - マージコンフリクトが発生した場合:
     ...
     b. `gh pr comment {PR_NUMBER} --repo {OWNER}/{REPO} --body 'マージコンフリクトが発生しました。手動での解決が必要です。'` でコメントを投稿する
...
Approve条件を満たす場合:
```bash
gh pr review {PR_NUMBER} --repo {OWNER}/{REPO} --approve --body '自動調査の結果、問題なしと判断しApproveしました。'
```
...
1. CI状態の確認:
   - `gh pr checks {PR_NUMBER} --repo {OWNER}/{REPO} --watch --timeout 600`
   - **タイムアウト** (10分経過) した場合: コメントを投稿して完了扱いにする
2. CIが失敗した場合:
   - `gh run list --branch {HEAD_BRANCH} --repo {OWNER}/{REPO} --limit 5 --json databaseId,name,status,conclusion` で失敗したrunを特定する
   - 失敗の詳細をコメントとして投稿する
