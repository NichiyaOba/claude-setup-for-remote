...
GitHub操作はすべて `gh` CLIコマンドを使用すること。MCP ツールは使用しない。
...
`gh pr comment {PR_NUMBER} --repo {REPO} --body '<コメント本文>'` でコメントを投稿する。
...
`gh pr review {PR_NUMBER} --repo {REPO} --approve --body '自動調査の結果、問題なしと判断しApproveしました。'` でApproveする。
